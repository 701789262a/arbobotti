...
data_train = data[(data.index >= '2021-03-25') & (data.index < '2021-03-29')]
print(data_train['TRT-BNB'].astype(str).astype(float))
data_train['TRT-BNB'].astype(str).astype(float).plot()
plt.show()
scaler = MinMaxScaler(feature_range=(0, 1))
data_train = np.array(data).reshape(-1, 1)
...
model.fit(x_train, y_train, epochs=2, batch_size=1280)
...
data_test = data[(data.index >= '2021-04-02') & (data.index < '2021-04-06')]
print(data_test['TRT-BNB'].astype(str).astype(float))
data_test['TRT-BNB'].astype(str).astype(float).plot()
plt.show()
scaler = MinMaxScaler(feature_range=(0, 1))
data_test = np.array(data).reshape(-1, 1)
